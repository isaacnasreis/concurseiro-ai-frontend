...
const progresso = computed(() => `Questão ${questaoAtualIndex.value + 1} de ${questoes.value.length}`);

const pontuacaoFinal = computed(() => {
  const acertos = questoes.value.reduce((total, questao, index) => {
    return respostasUsuario.value[index] === questao.resposta_correta ? total + 1 : total;
  }, 0);
  ...
  <main class="container">
      ...
      <form @submit.prevent="iniciarSimulado" class="form-container">
        <div class="form-group">
          ...
          <select id="materia" v-model="config.materia">
            <option v-for="materia in Object.keys(edital)" :key="materia" :value="materia">{{ materia }}</option>
          </select>
        </div>
        <div class="form-group">
          <label for="topico">Tópico</label>
          <select id="topico" v-model="config.topico">
            <option v-for="topico in topicosDisponiveis" :key="topico" :value="topico">{{ topico }}</option>
          </select>
        </div>
        <div class="form-group">
          <label for="quantidade">Número de Questões</label>
          <input type="number" id="quantidade" v-model.number="config.quantidade" min="1" max="10" />
        </div>
    ...
    <div v-else-if="etapa === 'execucao' && questaoAtual">
      ...
      <article class="questao-container">
        ...
        <ul class="alternativas">
          <li
            ...
            :class="{ selecionada: respostasUsuario[questaoAtualIndex] === alt }"
          >
      ...
      <div class="navegacao-simulado">
        <button @click="proximaQuestao" v-if="questaoAtualIndex < questoes.length - 1">Próxima</button>
    ...
    <div v-else-if="etapa === 'resultados'">
       ...
       <div class="resultado-placar">
          Você acertou
          <strong>{{ pontuacaoFinal.acertos }} de {{ pontuacaoFinal.total }}</strong>
...
.form-container, .questao-container, .resultado-placar { margin-bottom: 2rem; }
.form-group { display: flex; flex-direction: column; margin-bottom: 1rem; }
label { margin-bottom: 0.5rem; font-weight: bold; }
input, select { padding: 0.8rem; border: 1px solid #ccc; border-radius: 4px; }
button { padding: 1rem; background-color: #007bff; color: white; border: none; border-radius: 4px; cursor: pointer; font-size: 1rem; }
button:disabled { background-color: #aaa; }
.error-message { color: red; margin-top: 1rem; }
.progresso-bar { text-align: center; font-weight: bold; margin-bottom: 1rem; }
.enunciado { font-weight: bold; }
.alternativas { list-style: none; padding: 0; }
.alternativas li { padding: 0.8rem; border: 1px solid #ddd; margin-bottom: 0.5rem; border-radius: 4px; cursor: pointer; display: flex; align-items: center; }
.alternativas li:hover { background-color: #f0f0f0; color: #343a40; }
.alternativas li.selecionada { background-color: #dbeafe; border-color: #93c5fd; color: #343a40; }
.alternativas .letra { font-weight: bold; margin-right: 0.8rem; }
.navegacao-simulado { display: flex; justify-content: flex-end; margin-top: 1rem; }
.resultado-placar { text-align: center; font-size: 1.5rem; background-color: #f8f9fa; padding: 2rem; border-radius: 8px; color: #343a40; }
</style>